...
// Language detection panel elements
const langDetectionPanel = document.getElementById("langDetectionPanel");
const langPanelSelected = document.getElementById("langPanelSelected");
const langPanelConfidenceFill = document.getElementById("langPanelConfidenceFill");
const langPanelConfidenceValue = document.getElementById("langPanelConfidenceValue");
const langPanelReason = document.getElementById("langPanelReason");
const langPanelGap = document.getElementById("langPanelGap");
const langPanelCandidates = document.getElementById("langPanelCandidates");

function updateLangDetectionPanel(lines) {
  // Find the first line with lang_detection_info
  const lineWithLangInfo = (lines || []).find(line => line.lang_detection_info);

  if (!lineWithLangInfo || !lineWithLangInfo.lang_detection_info) {
    if (langDetectionPanel) langDetectionPanel.style.display = "none";
    return;
  }

  const info = lineWithLangInfo.lang_detection_info;
  const confidence = Math.round(info.confidence * 100);
  const gap = Math.round((info.gap_to_runner_up || 0) * 100);
  const isAmbiguous = info.is_ambiguous;

  // Show panel
  if (langDetectionPanel) langDetectionPanel.style.display = "block";

  // Update selected language
  if (langPanelSelected) {
    langPanelSelected.textContent = info.selected_lang.toUpperCase();
    langPanelSelected.className = `lang-panel-selected ${isAmbiguous ? 'ambiguous' : ''}`;
  }

  // Update confidence bar
  if (langPanelConfidenceFill) {
    langPanelConfidenceFill.style.width = `${confidence}%`;
    langPanelConfidenceFill.className = `confidence-fill ${isAmbiguous ? 'ambiguous' : ''}`;
  }
  if (langPanelConfidenceValue) {
    langPanelConfidenceValue.textContent = `${confidence}%`;
  }

  // Update reason and gap
  if (langPanelReason) {
    const reasonText = info.reason.replace(/_/g, ' ');
    langPanelReason.textContent = reasonText;
    langPanelReason.className = `lang-panel-reason ${isAmbiguous ? 'ambiguous' : ''}`;
  }
  if (langPanelGap) {
    langPanelGap.textContent = `Gap: ${gap}%`;
  }

  // Update candidates list
  if (langPanelCandidates && info.candidates) {
    const candidatesHtml = info.candidates.map((c, idx) => {
      const prob = Math.round(c.mean_prob * 100);
      const score = Math.round(c.weighted_score * 100);
      const isWinner = idx === 0;
      return `<div class="lang-candidate ${isWinner ? 'winner' : ''}">
        <span class="lang-candidate-code">${c.lang.toUpperCase()}</span>
        <div class="lang-candidate-bar-container">
          <div class="lang-candidate-bar" style="width: ${prob}%"></div>
        </div>
        <span class="lang-candidate-stats">${prob}% (${c.votes} votes, score: ${score}%)</span>
      </div>`;
    }).join('');
    langPanelCandidates.innerHTML = candidatesHtml;
  }
}
...
function renderLinesWithBuffer(
  ...
  if (current_status === "no_audio_detected") {
    ...
    updateLangDetectionPanel(null);
    return;
  }

  // Update language detection panel with detailed info
  updateLangDetectionPanel(lines);
    ...
    .map((item, idx) => {
      ...
      if (item.speaker === -2) {
        ...
        if (item.detected_language) {
          const langInfo = item.lang_detection_info;
          let langLabel = `<span class="label_language">
            ${languageIcon}
            <span class="lang-code">${item.detected_language.toUpperCase()}</span>`;

          // Add confidence bar and tooltip if lang_detection_info is available
          if (langInfo) {
            const confidence = Math.round(langInfo.confidence * 100);
            const isAmbiguous = langInfo.is_ambiguous;
            const reason = langInfo.reason || 'unknown';
            const gap = Math.round((langInfo.gap_to_runner_up || 0) * 100);

            // Confidence bar with tooltip
            const tooltipText = `Confidence: ${confidence}%, Gap: ${gap}%, Reason: ${reason}${isAmbiguous ? ' (AMBIGUOUS)' : ''}`;
            langLabel += `<span class="lang-confidence" title="${tooltipText}" style="--confidence: ${confidence}%">
              <span class="confidence-fill ${isAmbiguous ? 'ambiguous' : ''}"></span>
            </span>`;

            // Show candidates dropdown indicator if there are multiple candidates
            if (langInfo.candidates && langInfo.candidates.length > 1) {
              const candidatesStr = langInfo.candidates
                .slice(0, 3)
                .map(c => `${c.lang.toUpperCase()}: ${Math.round(c.mean_prob * 100)}%`)
                .join(', ');
              langLabel += `<span class="lang-candidates" title="Candidates: ${candidatesStr}">&#9660;</span>`;
            }
          }

          langLabel += `</span>`;
          speakerLabel += langLabel;
        }
      }
